...
class ThinpadTop extends RawModule {
    override val desiredName = s"lab3_top"
    ...
    withClock(clk_10M) {
        withReset(reset_btn) {
            ...
            withReset(reset_of_clock10M) {
                ...
                val regfile = Module(new RegFile(32,16))

                trigger.button := push_btn
                alu.io.a := regfile.io.rdata_a
                alu.io.b := regfile.io.rdata_b
                alu.io.op := controller.io.alu_op

                controller.io.dip_sw := dip_sw
                controller.io.trigger := trigger.trigger
                controller.io.alu_y := alu.io.y
                controller.io.rdata_a := regfile.io.rdata_a
                controller.io.rdata_b := regfile.io.rdata_b

                regfile.io.waddr := controller.io.waddr
                regfile.io.wdata := controller.io.wdata
                regfile.io.we := controller.io.we
                regfile.io.raddr_a := controller.io.raddr_a
                regfile.io.raddr_b := controller.io.raddr_b
                
                leds := controller.io.leds
            }
        }
    }
}
