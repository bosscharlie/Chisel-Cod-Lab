...
module Controller(
  ...
  reg  [31:0] ramAddr;
  reg  [31:0] uartData;
  reg  [7:0]  uartStatusReg;
  wire        _io_sel_o_T = stateReg == 4'h1;
  wire        _io_req_o_T_1 = stateReg == 4'h3;
  wire        _io_we_o_T = stateReg == 4'h5;
  wire        _io_sel_o_T_1 = stateReg == 4'h7;
  wire        _io_we_o_T_1 = stateReg == 4'h9;
    ...
    if (reset) begin
      ...
      automatic logic [15:0][3:0] _GEN =
        {{stateReg},
         ...
         {stateReg},
         {4'h0},
         {io_ack_i ? 4'hA : stateReg},
         {uartStatusReg[5] ? 4'h9 : 4'h7},
         {io_ack_i ? 4'h8 : stateReg},
         {4'h7},
         {io_ack_i ? 4'h6 : stateReg},
         {4'h5},
         {io_ack_i ? 4'h4 : stateReg},
         {{2'h0, uartStatusReg[0], 1'h1}},
         {io_ack_i ? 4'h2 : stateReg},
         {4'h1}};
      stateReg <= _GEN[stateReg];
      if (stateReg == 4'hA)
        ramAddr <= ramAddr + 32'h4;
      if (_io_req_o_T_1 & io_ack_i)
        uartData <= {24'h0, io_dat_i[7:0]};
      if ((_io_sel_o_T | stateReg == 4'h8) & io_ack_i)
        uartStatusReg <= io_dat_i[15:8];
    end
  end // always @(posedge)
  assign io_req_o =
    _io_sel_o_T | _io_req_o_T_1 | _io_we_o_T | _io_sel_o_T_1 | _io_we_o_T_1;
  assign io_we_o = _io_we_o_T | _io_we_o_T_1;
  assign io_adr_o =
    stateReg == 4'h1 | stateReg == 4'h7
      ? 32'h10000005
      : stateReg == 4'h3 | stateReg == 4'h9
          ? 32'h10000000
          : stateReg == 4'h5 ? ramAddr : 32'h0;
  assign io_dat_o = uartData;
  assign io_sel_o = {2'h0, _io_sel_o_T | _io_sel_o_T_1 ? 2'h2 : 2'h1};
endmodule
...
module WbMaster(
  ...
  output        io_wb_cyc_o,
                io_wb_stb_o,
  output [31:0] io_wb_adr_o,
                io_wb_dat_o,
  output [3:0]  io_wb_sel_o,
  output        io_wb_we_o
);

  reg  [1:0]  stateReg;
  reg  [31:0] rdata;
  wire        _io_wb_stb_o_T = stateReg == 2'h1;
  always @(posedge clock) begin
    if (reset) begin
      stateReg <= 2'h0;
      rdata <= 32'h0;
      ...
      automatic logic [3:0][1:0] _GEN =
        {{stateReg},
         {2'h0},
         {io_wb_ack_i ? 2'h2 : stateReg},
         {io_req_i ? 2'h1 : stateReg}};
      stateReg <= _GEN[stateReg];
      if (_io_wb_stb_o_T & io_wb_ack_i & ~io_we_i)
        rdata <=
          io_wb_dat_i
          & {{8{io_sel_i[3]}}, {8{io_sel_i[2]}}, {8{io_sel_i[1]}}, {8{io_sel_i[0]}}};
    end
  end // always @(posedge)
  assign io_ack_o = stateReg == 2'h2;
  assign io_dat_o = rdata;
  assign io_wb_cyc_o = _io_wb_stb_o_T;
  assign io_wb_stb_o = _io_wb_stb_o_T;
  assign io_wb_adr_o = io_adr_i;
  assign io_wb_dat_o = io_dat_i;
  assign io_wb_sel_o = io_sel_i;
  assign io_wb_we_o = io_we_i;
endmodule
...
// external module uart_controller

// external module wb_mux_3
...
module lab5_top(
  ...
  wire [31:0] _wb_mux_wbs2_adr_o;
  wire [31:0] _wb_mux_wbs2_dat_o;
  wire        _wb_mux_wbs2_we_o;
  wire [3:0]  _wb_mux_wbs2_sel_o;
  wire        _wb_mux_wbs2_stb_o;
  wire        _wb_mux_wbs2_cyc_o;
  wire        _uart_controller_wb_ack_o;
  wire [31:0] _uart_controller_wb_dat_o;
  ...
  wire        _wbMatser_io_ack_o;
  wire [31:0] _wbMatser_io_dat_o;
  wire        _wbMatser_io_wb_cyc_o;
  wire        _wbMatser_io_wb_stb_o;
  wire [31:0] _wbMatser_io_wb_adr_o;
  wire [31:0] _wbMatser_io_wb_dat_o;
  wire [3:0]  _wbMatser_io_wb_sel_o;
  wire        _wbMatser_io_wb_we_o;
  wire        _controller_io_req_o;
  wire        _controller_io_we_o;
  wire [31:0] _controller_io_adr_o;
  wire [31:0] _controller_io_dat_o;
  wire [3:0]  _controller_io_sel_o;
  ...
  Controller controller (
    ...
    .io_baseAddr (dip_sw),
    .io_ack_i    (_wbMatser_io_ack_o),
    .io_dat_i    (_wbMatser_io_dat_o),
    .io_req_o    (_controller_io_req_o),
    .io_we_o     (_controller_io_we_o),
    .io_adr_o    (_controller_io_adr_o),
    .io_dat_o    (_controller_io_dat_o),
    .io_sel_o    (_controller_io_sel_o)
  );
  WbMaster wbMatser (
    .clock       (_clock_gen_clk_out1),
    .reset       (reset_of_clock10M),
    .io_req_i    (_controller_io_req_o),
    .io_we_i     (_controller_io_we_o),
    .io_adr_i    (_controller_io_adr_o),
    .io_dat_i    (_controller_io_dat_o),
    .io_sel_i    (_controller_io_sel_o),
    .io_wb_ack_i (_wb_mux_wbm_ack_o),
    .io_wb_dat_i (_wb_mux_wbm_dat_o),
    .io_ack_o    (_wbMatser_io_ack_o),
    .io_dat_o    (_wbMatser_io_dat_o),
    .io_wb_cyc_o (_wbMatser_io_wb_cyc_o),
    .io_wb_stb_o (_wbMatser_io_wb_stb_o),
    .io_wb_adr_o (_wbMatser_io_wb_adr_o),
    .io_wb_dat_o (_wbMatser_io_wb_dat_o),
    .io_wb_sel_o (_wbMatser_io_wb_sel_o),
    .io_wb_we_o  (_wbMatser_io_wb_we_o)
  );
  ...
  uart_controller #(
    ...
    .uart_rxd_i (rxd),
    .wb_stb_i   (_wb_mux_wbs2_stb_o),
    .wb_cyc_i   (_wb_mux_wbs2_cyc_o),
    .wb_adr_i   (_wb_mux_wbs2_adr_o),
    .wb_we_i    (_wb_mux_wbs2_we_o),
    .wb_dat_i   (_wb_mux_wbs2_dat_o),
    .wb_sel_i   (_wb_mux_wbs2_sel_o),
    .uart_txd_o (txd),
    .wb_ack_o   (_uart_controller_wb_ack_o),
    .wb_dat_o   (_uart_controller_wb_dat_o)
  );
  wb_mux_3 wb_mux (
    .clk           (_clock_gen_clk_out1),
    .rst           (reset_of_clock10M),
    .wbm_adr_i     (_wbMatser_io_wb_adr_o),
    .wbm_dat_i     (_wbMatser_io_wb_dat_o),
    .wbm_we_i      (_wbMatser_io_wb_we_o),
    .wbm_sel_i     (_wbMatser_io_wb_sel_o),
    .wbm_stb_i     (_wbMatser_io_wb_stb_o),
    .wbm_cyc_i     (_wbMatser_io_wb_cyc_o),
    ...
    .wbs1_err_i    (1'h0),
    .wbs1_rty_i    (1'h0),
    .wbs2_addr     (32'h10000000),
    .wbs2_addr_msk (32'hFFFF0000),
    .wbs2_dat_i    (_uart_controller_wb_dat_o),
    .wbs2_ack_i    (_uart_controller_wb_ack_o),
    .wbs2_err_i    (1'h0),
    .wbs2_rty_i    (1'h0),
    ...
    .wbs2_adr_o    (_wb_mux_wbs2_adr_o),
    .wbs2_dat_o    (_wb_mux_wbs2_dat_o),
    .wbs2_we_o     (_wb_mux_wbs2_we_o),
    .wbs2_sel_o    (_wb_mux_wbs2_sel_o),
    .wbs2_stb_o    (_wb_mux_wbs2_stb_o),
    .wbs2_cyc_o    (_wb_mux_wbs2_cyc_o)
  );
  assign leds = 16'h0;
  assign dpy0 = 8'h0;
  assign dpy1 = 8'h0;
